<?php
  ...
  class sfRoute implements Serializable {
      ...
      foreach ($tokens as $token) {
        switch ($token[0]) {
          ...
          default:
            ...
            if ($segment = call_user_func_array([$this, 'generateFor' . ucfirst(array_shift($token))], array_merge([
              $optional,
              $parameters
            ], $token))) {
              $url[] = $segment;
    ...
    protected function fixSuffix () {
      $length = is_null($this->pattern) ? 0 : strlen($this->pattern);
      ...
      } else if (!is_null($this->pattern) and preg_match('#\.(?:' . $this->options['variable_prefix_regex'] . $this->options['variable_regex'] . '|' .
          $this->options['variable_content_regex'] . ')$#i', $this->pattern)) {
    ...
    public function __unserialize ($data) {
      ...
      $this->options = $data[2];
      $this->pattern = $data[3];
      $this->staticPrefix = $data[4];
      $this->regex = $data[5];
      $this->variables = $data[6];
      $this->defaults = $data[7];
      $this->requirements = $data[8];
      $this->suffix = $data[9];
      $this->customToken = $data[10];
      $this->compiled = true;
    }
  }